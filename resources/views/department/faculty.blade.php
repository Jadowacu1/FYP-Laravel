    ...
    <div class="row">
        ...
        <nav class="col-md-2 d-none d-md-block sidebar py-4 px-3" id="sidebar">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item"><a class="nav-link active" href="{{ route('department.faculty') }}"><i class="bi bi-journal-text me-2"></i><span> Faculty</span></a></li>
        ...
        <main class="col-md-10 ms-sm-auto col-lg-10 px-md-5 py-4 main-content">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="dashboard-title">Faculty Management</h2>
            ...
            <!-- Faculty Form -->
            <form action="{{ isset($faculty) ? route('updateFaculty', $faculty->FacultyCode) : route('addFacultyForm') }}" method="POST">
                @csrf
                @if(isset($faculty)) @method('PUT') @endif
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label for="FacultyName" class="form-label">Faculty Name</label>
                        <input type="text" name="FacultyName" class="form-control" value="{{ old('FacultyName', $faculty->FacultyName ?? '') }}" required>
                    </div>
                    <div class="col-md-4">
                        <label for="CampusId" class="form-label">Campus</label>
                        <select name="CampusId" class="form-select" required>
                            <option value="">-- Select Campus --</option>
                            @foreach($campuses as $campus)
                                <option value="{{ $campus->CampusId }}" {{ (isset($faculty) && $faculty->CampusId == $campus->CampusId) ? 'selected' : '' }}>
                                    {{ $campus->CampusName }}
                                </option>
                            @endforeach
                        </select>
                ...
                <button type="submit" class="btn btn-{{ isset($faculty) ? 'warning' : 'primary' }}">
                    {{ isset($faculty) ? 'Update Faculty' : 'Add Faculty' }}
                </button>
                @if(isset($faculty))
                    <a href="{{ route('department.faculty') }}" class="btn btn-secondary ms-2">Cancel</a>
                @endif
            </form>

            <!-- Faculty Table -->
            <h4 class="mt-5 mb-3">List of Faculties</h4>
                ...
                <tbody>
                    @forelse($faculties as $faculty)
                        <tr>
                            ...
                            <td>{{ $faculty->campus->CampusName }}</td>
                            <td>{{ $faculty->created_at->format('Y-m-d') }}</td>
                            <td>
                                <a href="{{ route('editFaculty', $faculty->FacultyCode) }}" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i> Edit</a>
                                <form action="{{ route('deleteFaculty', $faculty->FacultyCode) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this faculty?');">
                                    @csrf @method('DELETE')
